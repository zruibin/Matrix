   ...
   for(y=_text_y;y>-_text_y;y--){
      for(x=-_text_x;x<_text_x;x++){
         if(inSettings->text_light[p]>128 && inSettings->text_light[p+inSettings->text_x]<10) {
            /* White character */
            if (!mode) {
               draw_char(2, inSettings->text[p]+1,0.5,x,y, 
                  ((x>-_rtext_x-1 && x<_rtext_x )?inSettings->bump_pic[p]:8));
            /* Flare */
            } else {
               draw_flare(x,y, 
                  ((x>-_rtext_x-1 && x<_rtext_x )?inSettings->bump_pic[p]:8));
...
int mgl_scroll(int mode, MGLSettings * inSettings)
{
	int a, s=0;
...
int make_change(MGLSettings * inSettings)
{
    ...
    if (inSettings->mode2) {
        return mgl_scroll(inSettings->mode2,inSettings);
    }
...
void ourInit(int Width,int Height,MGLSettings * inSettings) 
{
   ...
   inSettings->text_x = ceil(70 * ((float)Width/Height));
	
	inSettings->pic_offset=0;//inSettings->text_x*text_y*7;

	initSettings(inSettings);

   /* Initializations */
   if (inSettings->text_x % 2 == 1) inSettings->text_x++;
   if (inSettings->text_x < 90) inSettings->text_x=90; /* Sanity check */
   ...
   for(a=0;a<inSettings->text_x;a++) {
      ...
      if (!a && inSettings->speed[a]==inSettings->speed[a-1]) inSettings->speed[a]=2; /* Collisions goto speed 3 */
   }

   /* Make Textures */
   glPixelTransferf(GL_GREEN_SCALE, 1.15f); /* Give green a bit of a boost */
   glBindTexture(GL_TEXTURE_2D,1);
   gluBuild2DMipmaps(GL_TEXTURE_2D,GL_RGBA8, 512, 256, inSettings->color,
